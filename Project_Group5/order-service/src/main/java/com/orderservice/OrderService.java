...
public class OrderService {
    ...
    public Order placeOrder(Order order) {
        ...
        try {
            ...
            String accountServiceUrl = "http://account-transaction-service/api/accounts/updateBalance/" + savedOrder.getUserId() + "?amount=" + (-totalCost);
            logger.info("Attempting balance update at: {}", accountServiceUrl);
            try {
                restTemplate.put(accountServiceUrl, null);
            } catch (HttpClientErrorException e) {
                logger.error("Balance update failed with status: {}, response: {}", e.getStatusCode(), e.getResponseBodyAsString());
                if (e.getStatusCode() == HttpStatus.NOT_FOUND) {
                    logger.info("Account not found for userId: {}, creating new account", savedOrder.getUserId());
                    String createAccountUrl = "http://account-transaction-service/api/accounts";
                    Map<String, Object> accountData = new HashMap<>();
                    accountData.put("userId", savedOrder.getUserId());
                    accountData.put("balance", totalCost + 10000); // Buffer
                    accountData.put("typeOfUser", "customer");
                    accountData.put("password", "default_password");
                    try {
                        logger.info("Sending POST to create account: {}", accountData);
                        restTemplate.postForObject(createAccountUrl, accountData, Map.class);
                        logger.info("Account created successfully");
                    } catch (Exception ex) {
                        logger.error("Failed to create account: {}", ex.getMessage(), ex);
                        throw ex;
                    }
                    logger.info("Retrying balance update");
                    restTemplate.put(accountServiceUrl, null);
                } else {
                    throw e;
                }
